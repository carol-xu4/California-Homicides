...
# ACS data -----------------------------------------------------------------
ddi_acs_ca = read_ipums_ddi("data/input/usa_00027.xml")
acs_ca= read_ipums_micro(ddi_acs_ca)

acs_ca = acs_ca %>% rename_with(tolower)
...
acs_ca = acs_ca %>%
  select(
    ...
    incss, incwelfr, incsupp, occ2010,
...
fwrite(acs_ca, "data/output/acs_california.csv")